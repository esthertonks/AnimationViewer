...
namespace render
{
	...
	RenderableNormalsList::RenderableNormalsList(
		...
		: Renderable(),
		m_normalsVertexListCreator(new NormalsVertexListCreator(mesh))
	{
	}
	...
	void RenderableNormalsList::PrepareForRendering() //FIXME idnetical to RenderableBoneList
	{
		// Create the VBO:
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(render::ColourVertex) * m_normalsVertexListCreator->GetNumVertsInList(), &m_normalsVertexListCreator->GetVertexList()[0], GL_STATIC_DRAW);
	...
	void RenderableNormalsList::Render( //FIXME identical to renderableBoneList
	...
	)
	{
		...
		if (m_normalsVertexListCreator->GetNumVertsInList() == 0)
		{
			return;
		}
		...
		if (programId != -1)
		{
			...
			glDrawArrays(GL_LINES, 0, m_normalsVertexListCreator->GetNumVertsInList());
			//glDrawArrays(GL_POINTS, 0, m_normalsVertexListCreator->GetNumVertsInList());
		}
